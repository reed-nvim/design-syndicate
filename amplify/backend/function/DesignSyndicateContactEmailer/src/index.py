...
def handler(event, context):
    ...
    origin = event['headers'].get('origin', '').lower()
    logger.info(f"🔹 [lambda_handler] origin: {origin}")
    logger.info(f"🔹 [lambda_handler] checking if origin is whitelisted")
    if 'syndicate' not in origin:
        logger.info(f"🔺 [lambda_handler] origin invalid")
        return {
            'statusCode': 400,
            'body': json.dumps({'message': 'Bad Request: Invalid origin'}),
            'headers': {
    ...
    logger.info(f"🔹 [lambda_handler] valid origin")
